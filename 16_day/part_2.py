...
# WARNING: I have no idea how this is correct but it works
class Node:
...
def main():
    ...
    output = runSim("AA", nodes, 26, 0, opened, [], valvesWithFlow, cache, False)

    newCache = {}
    output2 = runSim("AA", nodes, 26, 0, output[1], [], valvesWithFlow, newCache, False)

    print(output[0] + output2[0])

def runSim(currentNode, nodes, minutes, currentRate, opened, prev, valvesWithFlow, cache, openValve):
    if (currentNode, minutes, tuple(sorted(opened)), openValve) in cache:
        return cache[(currentNode, minutes, tuple(sorted(opened)), openValve)]

    if minutes <= 0 or len(opened) >= valvesWithFlow:
        return (currentRate, opened)
    ...
    for nextNode in node.connections:
        ...
        if maxOfTwo[0] > maxOutput:
            ...
            maxOpened = maxOfTwo[1]

    cache[(currentNode, minutes, tuple(sorted(opened)), openValve)] = (maxOutput, maxOpened)

    return (maxOutput, maxOpened)
